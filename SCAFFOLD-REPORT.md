...
Проект переименован в **Cubby UI** по решению Сергея уже по ходу этой работы; прежнее рабочее имя
в репозитории не встречается нигде — ни в путях, ни в текстах, ни в именах пакетов (проверено
grep'ом перед финальным коммитом).

Читаные источники: `~/Documents/Claude/активные/Cubby UI/AGENTS.md` и
`raw/2026-09-11-research-origin/{03-architecture.md, 04-yashik-inventory.md}` — только чтение,
как и `~/Documents/GitHub/altis` и папка «Ящик».
...
`git ls-remote origin` показал пустой репозиторий (ноль ссылок, код 0) — то есть репозиторий
существует и в него ещё ничего не клали, — после чего `git push -u origin main` прошёл: все пять
коммитов на GitHub, `main` отслеживает `origin/main`. `--force` не применялся нигде.

Одна оговорка про сеть: `~/.ssh/config` на этой машине уводит github.com на `ssh.github.com:443`
(«порт 22 у провайдера режется»), и в песочнице этот маршрут рвётся на обмене ключами. Пуш сделан
разовым `GIT_SSH_COMMAND="ssh -p 22 -o HostName=github.com -i ~/.ssh/github"`, без правки
пользовательского `~/.ssh/config`. У Сергея его обычный маршрут через 443, скорее всего, работает
как раньше; если нет — это отдельная проблема сети, а не репозитория.
